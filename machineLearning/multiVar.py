from numpy import loadtxt, zeros, ones, array, linspace, logspace, mean, std, arange,sys
...
print foodItem,weekday

foodValue = float(abs(hash(hash(foodItem)/10000000000000000)))
# weekday=5

quant = array([1.0,   ((foodValue - mean_r[0]) / std_r[0]), ((weekday - mean_r[1]) / std_r[1])]).dot(theta)
print 'Predicted quantity: %f' % (quant)
